  ...
  const renderItem = ({ item }: any) => {
    ...
    return (
      <View style={[styles.msgRow, isMe && styles.msgRowRight]}>
        {!isMe && (
          <Text style={[styles.senderLabel, { color: colors.gray400 }]}>Customer</Text>
        )}
        <View
          style={[
            styles.bubble,
  ...
  return (
    <SafeAreaView style={[styles.container, { backgroundColor: colors.black }]}>
      <View style={[styles.header, { borderBottomColor: colors.darkBorder }]}>
        ...
        <Text style={[styles.title, { color: colors.white }]}>{customerName || "Customer"}</Text>
        <View style={{ width: 60 }} />
      </View>
